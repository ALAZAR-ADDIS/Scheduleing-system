  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <>
      <div className={`flex items-center gap-2.5 border-b border-gray-100 shrink-0 ${(!mobile && collapsed) ? "px-3.5 py-5 justify-center" : "px-5 py-5"}`}>
        ...
        {(mobile || !collapsed) && (
          <div>
            <p className="text-sm font-bold text-gray-800 leading-tight">Employee Portal</p>
            <p className="text-xs text-orange-500 font-medium">AppointManager</p>
          </div>
        )}
        {mobile && (
      ...
      <div className={`pb-5 ${(!mobile && collapsed) ? "px-2" : "px-3"}`}>
        <button type="button" onClick={logout}
          ...
          className={`flex items-center gap-3 w-full rounded-xl text-sm font-medium text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors ${(!mobile && collapsed) ? "px-2.5 py-2.5 justify-center" : "px-3 py-2.5"}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <div className="md:hidden flex items-center gap-3 bg-white border-b border-gray-100 px-4 py-3 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-gray-800">Employee Portal</span>
          </div>
        </div>
        {children}
      </div>
